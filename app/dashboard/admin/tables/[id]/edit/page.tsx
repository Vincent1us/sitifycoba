...
import { getTables, updateTable, getRestaurants } from '@/lib/api';
...
export default function EditTablePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tablesData, restaurantsData] = await Promise.all([getTables(), getRestaurants()]);
        setRestaurants(restaurantsData || []);
        const table = tablesData.find((t: any) => t.id === parseInt(id as string));
        if (table) {
          setFormData({
            tableNumber: table.tableNumber || '',
            capacity: table.capacity || '',
            locationType: table.locationType || 'INDOOR',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await updateTable(parseInt(id as string), {
      ...
      setError(err.response?.data?.message || 'Gagal mengupdate meja');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Edit Meja</h1>
      </div>
